 ...
 * Phase 1 default: fly to Madrid, Spain on load.
 */
export const CAMERA_PRESETS = {
  austin: {
    destination: Cesium.Cartesian3.fromDegrees(-3.7038, 40.4168, 800),
    orientation: {
...
/**
 * Set camera to Madrid on load with a cinematic fly-in.
 * @returns {Function} Cancels the pending or active startup flight.
 */
export function flyToAustin(viewer) {
  // Start from a high altitude, then fly down
  viewer.camera.setView({
    destination: Cesium.Cartesian3.fromDegrees(-3.7038, 40.4168, 25000),
    orientation: {
      heading: Cesium.Math.toRadians(0),
  ...
  const timer = setTimeout(() => {
    ...
    viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(-3.7038, 40.4168, 600),
      orientation: {
