...
/**
 * Extra request headers for known CCTV image hosts that expect a normal
 * browser navigation rather than a minimal server-side fetch.
 *
 * MC30's public camera JPEGs are directly viewable in a browser, but the
 * upstream may treat a bare Node fetch differently. Keep this narrowly pinned
 * to the official MC30 hosts; arbitrary registered URLs do not inherit these
 * headers.
 */
export function cctvUpstreamImageHeaders(url) {
  const headers = {
    'User-Agent': cctvUpstreamUserAgent(url),
    Accept: 'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8',
  };
  try {
    const hostname = new URL(url).hostname.toLowerCase();
    if (hostname === 'www.mc30.es' || hostname === 'mc30.es') {
      headers.Referer = 'https://www.mc30.es/';
...
/**
 * MC30 fallback transport.
 *
 * Some MC30 installations serve the JPEG correctly to a normal browser but
 * reject Node's built-in fetch transport. Use the Node HTTPS client as a
 * second, narrowly-scoped transport and force IPv4 so the proxy follows the
 * same public endpoint without weakening the source registry.
 */
function fetchMc30ImageViaHttps(url, { timeoutMs, maxBytes }) {
    ...
    const request = https.request(
      ...
      (response) => {
        ...
        if (status >= 300 && status < 400 && location) {
          ...
          fetchMc30ImageViaHttps(next.toString(), { timeoutMs, maxBytes }).then(
            resolve,
          );
          return;
        }
        ...
        response.on('end', () => {
          ...
          finish({
            ...
            contentType: contentType.split(';', 1)[0].trim() || 'image/jpeg',
          });
        });
...
export async function fetchCctvImageFromUpstream(
  ...
  try {
    const upstream = await fetchWithinHost(
      url,
      {
        headers: cctvUpstreamImageHeaders(url),
        signal: controller.signal,
      },
      fetchImpl,
    );
    if (upstream) {
      ...
      if (upstream.ok && contentType.startsWith('image/')) {
        ...
        if (body) return { ok: true, body, contentType };
      }
    }

    // MC30 is the only known image provider in this catalog where the same
    // public JPEG can load in a desktop browser while Node fetch is rejected.
    // Retry only that exact host with the native HTTPS transport.
    try {
      const host = new URL(url).hostname.toLowerCase();
      if (host === 'www.mc30.es' || host === 'mc30.es') {
        controller.abort();
        return await fetchMc30ImageViaHttps(url, { timeoutMs, maxBytes });
      }
    } catch {
      /* invalid URL */
    }
